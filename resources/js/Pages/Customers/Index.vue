...
watch(
  () => formSearch.value,
  throttle(function () {
    router.get("/customers", pickBy(formSearch.value), { preserveState: true });
  }, 150),
  {
    deep: true,
  }
...
const deleteCustomer = () => {
  form.delete(route("customers.destroy", customerADelete.value), {
    preserveScroll: true,
...
<template>
            ...
            <template #default>
              <TableRow v-for="customer in customers.data" :key="customer.id">
                ...
                <TableDataCell>
                  ...
                  <a
                    @click="confirmCustomerDeletion(customer)"
    ...
    <pagination :registers="customers" />
    ...
    <DialogConfirmation :show="confirmingItemDeletion" @close="closeModal">
      ...
      <template #content>
        ¿Estás seguro de que quieres eliminar este Cliente? Una vez que su cuenta se
